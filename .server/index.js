...
app.get('*', (req,res) => {
    ...
    if(file.isDirectory()){
        /*
        if(path.at(-1) != '/'){
            var temppath = join(path,'index.html');

            file = statSync(temppath,{throwIfNoEntry:false});
            if(file !== undefined && file.isFile()){
                res.type(extname(temppath)).sendFile(temppath);
                return;
            }
        }*/
        ...
        var tableString = '';
        if(req.path.length > 1) tableString = `<tr><td>📁</td><td><a href="${join(req.path,'..') + ((showHidden && iD) ? '?h=t':'')}">..</a></td><td>${readdirSync(join(path,'..')).length}</td></tr>`;
        ...
        for(let fil of files){
            ...
            tableString += `<tr><td>${iD?'📁':'📄'}</td><td><a href="${join(req.path,fil) + ((showHidden && iD) ? '?h=t':'')}">${fil}</a></td><td>${iD?readdirSync(npath).length+' files':f.size+' B'}</td></tr>`;
        }
        var replTable = {
            '{dirname}': path,
            '{table}':  tableString
